...
while (
    ...
    runtime.pc.programPointer != 0xFFFF - 1 && c < 20) {
    try {
...
instruction: ${runtime.pc.mem[runtime.pc.programPointer]}`
        ...
        const args = [];
        const pc = runtime.pc.programPointer - 1
        ...
        if (Deno.args.includes('-d'))
            console.debug(pc.toString(16).padStart(4, '0'), runtime.pc.registers.map(r => r.toString(16).padStart(4, '0')).join(', '), definition, instruction, runtime.pc.mem[runtime.pc.programPointer + 1])
        // runtime.pc.programPointer++
        c++
    } catch (error) {
        console.error(error);
